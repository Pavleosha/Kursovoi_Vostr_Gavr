...
def cr_mat_body(x0, y0, r0, r1, r2, r3, r4, n):
    ...
    def get_crd(r):
        return x0 + r * np.cos(phi), y0 + r * np.sin(phi)       # Непосредственный переход
    ...
    def add_mpoints(x, y):
        ...
        mat_points.append(model.MatPoint(x, y, fx(t, x), fy(t, y), x, y, t))
...
def move_mat_body(time, h, mb, a, b, c):
    ...
    for i in range(len(mb.mat_points)):
        x0 = mb.mat_points[i].x0        # Обращение к координате х0 i-той мат. точки материального тела
    ...
    for n in range(int(time/h)):
        ...
        for i in range(7):
            for j in range(7):
                x = x_s[i, j]
                y = y_s[i, j]
                space_points.append(model.SpacePoint(x, y, fx(t, x), fy(t, y), t))
